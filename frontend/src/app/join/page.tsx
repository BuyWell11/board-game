...
const JoinRoom = () => {
  ...
  const onSubmit = handleSubmit((formData) => {
    if (formData.nickname.trim() === '' || formData.roomCode.trim() === '') {
      return;
    }

    socket.emit('join-room', formData.roomCode.toLocaleUpperCase(), formData.nickname, (response: Room) => {
      if (response.error) {
  ...
  return (
    <Box className={'page'}>
      <form onSubmit={onSubmit} className={'create-join-form'}>
        ...
        <Field label="Room code" invalid={!!errors.nickname} errorText={errors.nickname?.message}>
          <Input {...register('roomCode', { required: 'Room code is required' })} />
        </Field>
